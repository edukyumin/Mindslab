...
8. 아래의 `Go` 나 `Preprocessing` 을 누르고 기다린다
   ...
   5. 여기서 PID확인한 후에 kill 해주고 새로띄워줘야한다 
...
1. Shell 6번서버 켜기
   ...
   7. `_re` 파일이 생성 되었다면 `https://docs.google.com/spreadsheets/d/1wcCiTjAGtTAfsUdDG8yxeaHuUdiN-7e9cSzmEpln8iU/edit#gid=0` 이곳에 들어가서 나의 가장 좋았던 taco2 와 waveglow의 `파일명`, `파라미터`모두 적어놓는다
...
*  http://106.242.34.179:3000/
*  ID: minds
*  PW: minds123!@#
*  접속 후 상단의 HOME > SS-ENGEDU-STT> 상단의 항목조작
...
5. `docker exec -it w2l_samsung_103 bash`
   ...
   * 이때 똑같이 서버 하나 더 켜서 `nvidia-smi` 쳐봐서 떠있는 Server의 pid를 기억해놓자
   * 그리고 나서 아래의 7번까지 끝난다음에 위의 pid는 살려두고 아래 6을 실행해서 생기는 pid는 `sudo kill -9 {pid}` 해주자
...
> 40 or 41번서버에서 추출 (주로41에서 햇음)

1. 원하는 서버에 `MySQL Workbench` 로 접속
...
2. 좌측 상단의 File 아래에 있는 `+` 를 눌러서 Quary 문 에디터를 연다

3. Quary문을 작성해서 원하는 값을 뽑는다. 
...
4. SQL Quary문예시
   ...
   * 21년 데이터 뽑기
     ...
     * ```sql
       select * FROM mindsedu.dl_evaluation_user_tb 
       where created_dtm >= str_to_date('2021-01-01', '%Y-%m-%d')
       ```

   * 21년 데이터 중 오답률이 높은 문장들을 뽑기

     * ```sql
       select * 
       from
       (select X.answer_text, sum(X.count) as num_wrong , count(X.count) as num, sum(X.count)/count(X.count) as wrong_ratio
       from
       (SELECT
       answer_text,
       case when replace(replace(replace(replace(replace(user_text, '.', ''),'?',''),'!',''),',',''),"'","") != replace(replace(replace(replace(replace(answer_text, '.', ''),'?',''),'!',''),',',''),"'","") then 1
       else 0 end as 'count',
       created_dtm
       FROM mindsedu.dl_evaluation_user_tb where created_dtm >= str_to_date('2021-01-01', '%Y-%m-%d'))X
       group by X.answer_text)Y
       where length(Y.answer_text) >= 10 and Y.num_wrong >= 10000
       order by Y.wrong_ratio DESC
       
       ```

   * answer_text 기준으로 파일의 경로 뽑기

     * ```sql
       select concat('/data1/record/stt/evaluation/eng/',file_path, file_wave_name), answer_text, user_text, created_dtm
       FROM mindsedu.dl_evaluation_user_tb
       where answer_text = "I'm from Korea." and created_dtm >= str_to_date('2021-01-01', '%Y-%m-%d')
